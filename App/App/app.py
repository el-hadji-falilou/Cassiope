...
MINICIPHER_PATH = os.path.abspath("minicipher.py")
COMPUTE_PROBAS_PATH = os.path.abspath("compute_proba.py")
FIND_KEY = os.path.abspath("find_key.py")
...
def validate_input(question_id, side):
    ...
    if question_id == 5:
        ...
        for i in range(16):
            if student_data['proba'][f'val{i}'] != expected_data['proba'][f'val{i}']:
                errors.append({
                    "type": "proba",
                    "index": i+16,  # Offset pour la 2ème ligne
    ...
    # Validation pour les autres questions (non-tableau)
    else:
        is_correct = student_data == expected_data
        return jsonify({
